...
#import sys
#sys.path.append("/home/GMI/rahul.pisupati/Downloads/PyGWAS-master/pygwas/core")
import genotype

import scipy

def AltAlleleFreq(snp):
  snps_array = numpy.copy(snp)
  ...
  return AlleFreq, snps_array, info_array

def RefAlleleFreq(snp):
  snps_array = numpy.copy(snp)
  info_array = numpy.copy(snps_array)
  snps_array[snps_array == 1] = -1
  snps_array[snps_array == 0] = 1
  snps_array[snps_array == -1] = 0
  info_array[info_array == 0] = 1
  info_array[info_array == -1] = 0
  AlleFreq = numpy.array(numpy.sum(snps_array, axis= 1), dtype=float)/numpy.sum(info_array, axis= 1)
...
for snp in snps:
  ...
  chunk_i = chunk_i + 1
  (Freq, snps_array, info_array) = AltAlleleFreq(snp)
#  (Freq, snps_array, info_array) = RefAlleleFreq(snp)
  rareInd = numpy.where(Freq < options.allelFreq)[0]
