...
option_list = list(
  make_option(c("-e", "--ecosMerged"), type="character", default=NULL, help="Path to txt file with merged ecotype IDs", metavar="character"),
  ...
  make_option(c("-a", "--accFile"), type="character", default=NULL, help="Path to the accessions file for each position", metavar="character"),
...
pathechotypeIDs = opt$ecosMerged
outFile = opt$outFile
pathacc = opt$accFile
folid = opt$folID
...
mergedAcc <- read.table(pathechotypeIDs, header = F, sep="\t")
AssignedAcc <- read.table(pathacc, header = T, sep="\t", as.is = T)
...
for (file in allScoreFiles){
  ...
  #Converting the name of file into rows and columns, dont worry about these now Ilka added barcodes in the master list
  if (length(which(colnames(AssignedAcc) == "row")) > 0){
    tempnum <- as.numeric(strsplit(name, "_")[[1]][1])-700
    tempid <- chartr("12345678","ABCDEFGH",sub("50","",strsplit(name, "_")[[1]][2]))
    accass <- as.character(AssignedAcc$sample.description[which(AssignedAcc$row == tempid & AssignedAcc$column == tempnum)])
  } else {
    tempid <- strsplit(name, "_")[[1]][2]
    tempnum <- strsplit(name, "_")[[1]][1]
    accass <- as.character(AssignedAcc$sample.description[which(AssignedAcc$barcode1 == tempnum & AssignedAcc$barcode2 == tempid)])
  }
...
#  snps <- as.numeric(numSNPs$V1[which(numSNPs$V2 == name)])
  ...
  choicenum <- which(ScoreAcc$V1[ranks] == as.character(accass))

  if (length(choicenum) == 0){
    pmerged <- as.character(mergedAcc$V1[grep(accass, mergedAcc$V1, value=T)]) # Might be merged
    if (length(pmerged)){
      if(length(grep(topacc, pmerged, value=T))){
        accass <- topacc
        choicenum = "1"
      } else {
...
#   }
  ...
  TopHitsNumber <- c(TopHitsNumber, length(which(ScoreAcc$V6 < 3.841)))
