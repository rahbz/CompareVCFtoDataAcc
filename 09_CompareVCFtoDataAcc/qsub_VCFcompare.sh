...
# Simulated SNP matrix added a new matrix
hdf5="/lustre/scratch/projects/the1001genomes/VCF_1135g/simulated_1136accessions.made_col-0.hdf5"
hdf5_acc="/lustre/scratch/projects/the1001genomes/VCF_1135g/simulated_1136accessions.made_col-0.acc.hdf5"

# Removed SNPs from empty wells
#hdf5="/lustre/scratch/projects/the1001genomes/forRahul/empty_wells/1135g_SNP_BIALLELIC.hetfiltered.filteredSNPs_emptywells.4Dec2015.hdf5"
#hdf5_acc="/lustre/scratch/projects/the1001genomes/forRahul/empty_wells/1135g_SNP_BIALLELIC.hetfiltered.filteredSNPs_emptywells.4Dec2015.acc.hdf5"
...
#outFolder="newRefScore_VCF1135"
#outFolder="250k_Imputed"
#outFolder="wholeImputed"
#outFolder="newSimulatedScore_VCF1135_simulated1136Acc"
outFolder="newSimulatedScore_VCF1135_simulated1136Acc_made_col-0"
#outFolder="newSimulatedScore_VCF1135_removedSNPs_fromEmptywells"
mkdir $outFolder
...
#	grep -v "^#" $1.filter.gvcf | awk -v qth=$qthres '$6 > qth && $NF ~ /^1\/1|^0\/1|^1\/0|^0\/0/ {print $1 "\t" $2 "\t" substr($NF,1,3)}' | sed 's/^Chr//' > $PBS_O_WORKDIR/$outFolder/$1.pos.txt
	...
	module load ipython
#	python ~/MyScripts/CompareVCFtoDataAcc/09_CompareVCFtoDataAcc/01_CalcHammingDist.py -d $hdf5 -e $hdf5_acc -p $PBS_O_WORKDIR/$outFolder/$1.pos.txt -o $PBS_O_WORKDIR/$outFolder/$1.ScoreAcc.txt -r $PBS_O_WORKDIR/$outFolder/$1.refScore.txt > $outFolder/logs/log.oVCFc.$inFile 2> $outFolder/logs/log.oVCFc.$inFile
	python ~/MyScripts/CompareVCFtoDataAcc/09_CompareVCFtoDataAcc/01_CalcHammingDist.py -d $hdf5 -e $hdf5_acc -p $PBS_O_WORKDIR/$1.pos.txt -o $PBS_O_WORKDIR/$outFolder/$1.ScoreAcc.txt -r $PBS_O_WORKDIR/$outFolder/$1.refScore.txt > $outFolder/logs/log.oVCFc.$inFile 2> $outFolder/logs/log.oVCFc.$inFile
}
