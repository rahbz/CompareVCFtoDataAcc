...
rm $inFile.ScoreAcc.txt $inFile.refScore.txt
...
#	grep -v "^#" $1.filter.vcf | awk -v qth=$qthres '$6 > qth && $NF ~ /^1\/1|^0\/1|^1\/0/ {print $1 "\t" $2 "\t" $(NF-2) "\t" $(NF-1) "\t" $NF}' | sed 's/^Chr//' > $1.pos.txt
	...
	module load pygwas
	module load pandas
	python ~/MyScripts/CompareVCFtoDataAcc/06_CompareVCFtoDataAcc/01_CalcHammingDist.py -d $hdf5 -e $hdf5_acc -t $totSNPs -p $1.pos.txt -o $1.ScoreAcc.txt -r $1.refScore.txt > logs/log.oVCFc.$i 2> logs/log.oVCFc.$i
}
