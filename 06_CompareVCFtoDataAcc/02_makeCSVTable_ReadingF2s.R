...
for (echscore in list){
  ...
  FracSNPs1 <- c(FracSNPs1, matsnps[1]/numsnps)
  FracSNPs2 <- c(FracSNPs2, matsnps[2]/matsnps[1])
...
#   HetFreq <- c(HetFreq, perhet)
  ...
  nextone <- (ScoreAcc$V4[order(-ScoreAcc$V4)][2] - ScoreAcc$V4[order(-ScoreAcc$V4)][3])/sd(ScoreAcc$V4)
  NextDist <- c(NextDist, nextone)
  # hist(ScoreAcc$V4, breaks = 50)
}

DF <- data.frame(FILENAME = Names, ExpectedParent1 = FouParents1, ExpectedParent2 = FouParents2, Estimate1 = as.numeric(FracSNPs1), Estimate2 = as.numeric(FracSNPs2), KinEstimate = as.numeric(KinEstimate), ScoreP1 = ScoreP1, ScoreP2 = ScoreP2, SNPsmatchedP1 = as.numeric(SNPsmat1), SNPsmatchedP2 = as.numeric(SNPsmat2), SNPsCalled = SNPsCalled, NextHit = NextDist, ThirdHit = FouParents3, FourthHit = FouParents4, FifthHit = FouParents5, SixthHit = FouParents6)

#DF <- data.frame(FILENAME = Names, ExpectedParent1 = FouParents1, ExpectedParent2 = FouParents2, Estimate1 = as.numeric(FracSNPs1), Estimate2 = as.numeric(FracSNPs2), KinEstimate = as.numeric(KinEstimate), HetFreq = as.numeric(HetFreq), ScoreP1 = ScoreP1, ScoreP2 = ScoreP2, SNPsmatchedP1 = as.numeric(SNPsmat1), SNPsmatchedP2 = as.numeric(SNPsmat2), SNPsCalled = SNPsCalled, NextHit = NextDist, ThirdHit = FouParents3, FourthHit = FouParents4, FifthHit = FouParents5, SixthHit = FouParents6)
