...
#	grep -v "^#" $1.filter.gvcf | awk -v qth=$qthres '$6 > qth && $NF ~ /^1\/1|^0\/1|^1\/0|^0\/0/ {print $1 "\t" $2 "\t" substr($NF,1,3)}' | sed 's/^Chr//' > $PBS_O_WORKDIR/$outFolder/$1.pos.txt
	...
	module load ipython
	python ~/MyScripts/CompareVCFtoDataAcc/11_CompareVCFtoDataAcc/01_CalcHammingDist.py -d $hdf5 -e $hdf5_acc -p $PBS_O_WORKDIR/$outFolder/$1.pos.txt -o $PBS_O_WORKDIR/$outFolder/$1.ScoreAcc.txt -r $PBS_O_WORKDIR/$outFolder/$1.refScore.txt > $outFolder/logs/log.oVCFc.$inFile 2> $outFolder/logs/log.oVCFc.$inFile
