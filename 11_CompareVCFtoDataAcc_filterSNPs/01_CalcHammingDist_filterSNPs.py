...
inOptions.add_option("-f", "--filterSNP", dest="filterSNP", help="SNP positions which should be considered", type="string")
...
filterSNP = pandas.read_table(options.filterSNP, header=None, usecols=[0,1])
### Club the columns and try to filter those sites out before taking them into consideration
npfilterSNP = numpy.core.records.fromarrays(numpy.column_stack((numpy.array(filterSNP[0], dtype=int), numpy.array(filterSNP[1], dtype=int))).T)
nptargetSNP = numpy.core.records.fromarrays(numpy.column_stack((numpy.array(allTargetSNPs[0], dtype=int), numpy.array(allTargetSNPs[1], dtype=int))).T)
filterInd = numpy.where(numpy.in1d(nptargetSNP, npfilterSNP))[0]

targetSNPs = allTargetSNPs.loc[filterInd]
print targetSNPs.shape
