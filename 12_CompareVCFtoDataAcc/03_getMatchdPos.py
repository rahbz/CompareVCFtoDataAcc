...
inOptions.add_option("-i", "--vcf_file", dest="vcfFile", help="VCF file of the sample, from haplotype caller", type="string")
inOptions.add_option("-a", "--acc_to_check", dest="accToCheck", help = "Top hit accession from the ScoreAcc file", type="string")
inOptions.add_option("-b", "--acc_id_to_check", dest="accIDToCheck", help = "Top hit accession index from the ScoreAcc file", type="int")
...
if options.accToCheck:
  Acc = numpy.where(GenotypeData.accessions == options.accToCheck)[0]
  AccSNPs = GenotypeData_acc.snps[:, Acc]
elif options.accIDToCheck:
  AccSNPs = GenotypeData_acc.snps[:, options.accIDToCheck]

logging.debug("Reading the VCF file")
...
for i in range(1,6):
  perchrTarInd = numpy.where(snpCHR == i)[0]
  perchrtarSNPpos = numpy.array(snpPOS[perchrTarInd])
  ...
  matchedTarGTs = snpGT[perchrTarInd[matchedTarInd]]
  ...
  logging.debug("%d NonMat, %d Mat, %s TotInfo", len(nmat), len(mat), len(AccTarSNPs)-len(ImpPos))
...
numpy.savetxt(options.outNonMat, numpy.column_stack((TotNonMatChr, TotNonMatPos)), fmt=('%d','%d'), delimiter ="\t" )
numpy.savetxt(options.outMat, numpy.column_stack((TotMatChr, TotMatPos)), fmt=('%d','%d'), delimiter ="\t")
